...
import { sortProducts, filterProducts, type SortOption } from "@/lib/product-sorting"
...
export default function AccessoriesPage() {
  ...
  const filteredProducts = useMemo(() => {
    const filtered = filterProducts(accessoryProducts || [], searchTerm, {
  ...
  }, [accessoryProducts, searchTerm, filters, sortBy])
  ...
  const handleCompareToggle = (productSlug: string) => {
    ...
    <div className="min-h-screen">
      ...
      <main>
        ...
        <section className="py-16 bg-gradient-to-br from-background to-muted/30">
          <div className="container mx-auto px-4">
            ...
            <div className="max-w-3xl">
              ...
              <h1 className="text-4xl lg:text-5xl font-bold mb-6 text-balance">Complete Your Outdoor Look</h1>
              <p className="text-lg text-muted-foreground text-pretty">
                Essential accessories crafted from the same premium materials as our garments. Perfect for completing
                your outdoor collection and expanding your product range.
              </p>
            </div>
          </div>
        </section>
          ...
          <section className="py-6 bg-muted/30 border-b">
            <div className="container mx-auto px-4">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {/* Category Filter */}
                <div>
                  <Label className="text-sm font-medium mb-3 block">Sub-Category</Label>
                  <div className="space-y-2">
                    {["Hats", "Protective Gear"].map((category) => (
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        id="crushable"
                        checked={filters.hardware.includes("Crushable")}
                        onCheckedChange={(checked) => {
                          if (checked) {
                            updateFilter("hardware", [...filters.hardware, "Crushable"])
                          } else {
                            updateFilter(
                              "hardware",
                              filters.hardware.filter((h) => h !== "Crushable"),
                            )
                          }
                        }}
                      />
            ...
            <div className="container mx-auto px-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <Button variant="secondary" size="sm" onClick={() => setShowComparison(true)}>
                    Compare Now
                  </Button>
          ...
          <div className="container mx-auto px-4">
            <div className="mb-6 flex items-center justify-between">
              <p className="text-sm text-muted-foreground">
                Showing {filteredProducts?.length} of {accessoryProducts?.length} products
              </p>
            </div>
            ...
            <div className={viewMode === "grid" ? "grid md:grid-cols-2 lg:grid-cols-3 gap-8" : "space-y-6"}>
              {(filteredProducts || []).map((product) => (
                <ProductCard
                  ...
                  href={`/products/accessories/${product.slug}`}
                />
              ))}
            </div>
        ...
        {showComparison && compareList.length > 0 && (
          <ProductComparison
            products={(filteredProducts || []).filter((p) => compareList.includes(p.slug))}
            onClose={() => setShowComparison(false)}
            onClearSelection={() => setCompareList([])}
          />
        )}
        ...
        <section className="py-16 bg-card">
          <div className="container mx-auto px-4">
            <div className="max-w-3xl mx-auto text-center">
              <h2 className="text-2xl lg:text-3xl font-bold mb-4">Bundle Opportunities Available</h2>
              <p className="text-muted-foreground mb-6">
                Create complete collections by combining our garments with matching accessories. We offer attractive
                pricing for bundled orders.
              </p>
              <Button size="lg" asChild>
                <Link href="/contact">Explore Bundle Options</Link>
              </Button>
            </div>
          </div>
        </section>
